...
for (let i = 1; i <= n; i++) {
  if (i === 1) {
    ...
    minSteps[i] = Math.min(minSteps[i - 1], minSteps[i / 2], minSteps[i / 3]) + 1;
  } else if (i % 2 === 0) {
    minSteps[i] = Math.min(minSteps[i - 1], minSteps[i / 2]) + 1;
  } else if (i % 3 === 0) {
    minSteps[i] = Math.min(minSteps[i - 1], minSteps[i / 3]) + 1;
  } else {
