...
const height = nextInt();
const MOD = 10 ** 6;

// count[h] = количество способов построить башню высоты h
const count = Array(height + 12).fill(0);
count[10] = 1;
count[11] = 1;
count[12] = 1;

for (let h = 13; h <= height; h++) {
  count[h] = (count[h - 10] + count[h - 11] + count[h - 12]) % MOD;
}

console.log(count[height] * 2 % MOD);
