...
for (let r = 1; r <= sizeR; r++) {
  for (let c = 1; c <= sizeC; c++) {
    wins[r][c] = false; // предполагаем, что из текущей клетки нельзя победить
  
    // проверяем, может ли ферзь победить, начав игру из текущей клетки
    for (let nextRow = 1; nextRow < r && !wins[r][c]; nextRow++) {
      if (!wins[nextRow][c]) {
        wins[r][c] = true; // если можно, устанавливаем, что победа возможна из текущей клетки
      }
    }

    // проверяем для горизонтальных ходов
    for (let nextColumn = 1; nextColumn < c && !wins[r][c]; nextColumn++) {
      if (!wins[r][nextColumn]) {
        wins[r][c] = true;
      }
    }

    // проверяем для диагональных ходов
    for (let shift = 1; r - shift >= 1 && c - shift >= 1 && !wins[r][c]; shift++) {
      if (!wins[r - shift][c - shift]) {
        wins[r][c] = true;
      }
    }
  }
